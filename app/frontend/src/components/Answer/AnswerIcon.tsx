...
import { BuildingMultiple24Filled, Globe24Filled, Link20Filled, Sparkle24Filled, BuildingGovernment24Filled } from "@fluentui/react-icons";
...
export const AnswerIcon: React.FC<AnswerIconProps> = ({ approach }) => {
    if (approach == Approaches.ChatWebRetrieveRead) {
        ...
        return <div className={styles.answerLogoWork}><BuildingMultiple24Filled primaryFill={"rgba(27, 74, 239, 1)"} aria-hidden="true" aria-label="Work Answer logo" /> Common Platform (Guidance)</div>;
        }
    else if (approach == Approaches.TabularDataAssistant) {
        return <div className={styles.answerLogoWork}><BuildingGovernment24Filled primaryFill={"rgba(27, 74, 239, 1)"} aria-hidden="true" aria-label="Work Answer logo" /> Common Platform (CJS)</div>;
        }
